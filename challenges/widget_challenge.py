# uses GUI from script_GUI.py challenge

# Requirements: must use askdirectory() method
# must have function linked to button that allows user to select file path
# must print selected path to entry widget in GUI
...
def get_directory(entry_widget):
  directory = filedialog.askdirectory()
  entry_widget.delete(0, END)
  entry_widget.insert(0, directory)
  # entry_widget.configure(textvariable=v)  

# create and position widgets for the master window
def load_gui_content(frame):
  frame.btn_browse_1 = Button(frame.master, text='Browse...', command=lambda: get_directory(frame.input_1))
